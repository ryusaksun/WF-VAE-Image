...
from ..modeling_aebase import ImageBaseAE
from ..modules import (
    Conv2d,
    Downsample,
...
class WFDownBlockImage(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        self.in_flow_conv = Conv2d(12, energy_flow_size, kernel_size=3, stride=1, padding=1)
        ...
        self.out_res_block = res_block(
            in_channels=in_channels + energy_flow_size,
...
class WFUpBlockImage(nn.Module):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super().__init__()
        assert num_res_blocks >= 2, "num_res_blocks should be >= 2"
        self.energy_flow_size = energy_flow_size
...
class EncoderImage(ImageBaseAE):
    @register_to_config
    def __init__(
        self,
        latent_dim: int = 8,
        ...
        mid_layers_type: List[str] = ["ResnetBlock2D", "AttnBlock2D", "ResnetBlock2D"],
        down_layer_res_type: List[str] = ["ResnetBlock2D", "ResnetBlock2D"],
    ) -> None:
        super().__init__()
        ...
        for idx, down_res_type in enumerate(down_layer_res_type):
            down_block = WFDownBlockImage(
        ...
        for mid_layer_type in mid_layers_type:
            layer_cls = resolve_str_to_obj(mid_layer_type)
            if "Attn" in mid_layer_type:
                mid_layers.append(layer_cls(in_channels=base_channels[-1], norm_type=norm_type))
            else:
                mid_layers.append(
                    layer_cls(
        ...
        self.conv_out = Conv2d(base_channels[-1], latent_dim * 2, kernel_size=3, stride=1, padding=1)
...
class DecoderImage(ImageBaseAE):
    @register_to_config
    def __init__(
        self,
        latent_dim: int = 8,
        ...
        base_channels: List[int] = [128, 256, 512],
        mid_layers_type: List[str] = ["ResnetBlock2D", "AttnBlock2D", "ResnetBlock2D"],
        up_layer_res_type: List[str] = ["ResnetBlock2D", "ResnetBlock2D"],
    ) -> None:
        super().__init__()
        self.energy_flow_size = energy_flow_size
        self.conv_in = Conv2d(latent_dim, base_channels[-1], kernel_size=3, stride=1, padding=1)
        ...
        for idx, up_res_type in enumerate(reversed(up_layer_res_type)):
            source_idx = len(up_layer_res_type) - idx
            up_block = WFUpBlockImage(
                in_channels=base_channels[source_idx],
                out_channels=base_channels[source_idx - 1],
                energy_flow_size=energy_flow_size,
                num_res_blocks=num_resblocks,
...
@ModelRegistry.register("WFVAE2Image")
class WFVAE2ImageModel(ImageBaseAE):
    @register_to_config
    def __init__(
        self,
        latent_dim: int = 4,
        ...
        layer_res_type: List[str] = ["ResnetBlock2D", "ResnetBlock2D"],
